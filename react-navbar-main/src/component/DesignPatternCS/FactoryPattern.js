...
import {CDFactoryPattern,EXFactoryPattern,HTFactoryPattern} from "../../window/CSWindow/FactoryModal.js"
import "../../css/pre.css";
...
const FactoryPattern = () => {
  ...
  useEffect(() => {
    fetchAndUpdateDisplayContent(5); // 初始渲染時加載 templateId 為 5 的模板
  ...
  const handleGenerateCode = async () => {
    try {
      const response = await fetch(`http://localhost:3002/api/templates/5`);
      const data = await response.json();
        ...
        <div class="col-md-12">
          <div class="cardt">
            <div class="cardt-info">
              <p class="titlet">Factory Pattern</p>
            </div>
          </div>
        </div>
      </div>
            ...
            <div class="card-header">
              <div class="navigation">
                <ul>
                  <li
                    class="list active"
                    onClick={() =>fetchAndUpdateDisplayContent(5)} >
                    <span class="icon">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="25"
                  ...
                  <li
                    class="list"
                    onClick={() =>fetchAndUpdateDisplayContent(6)} > 
                    <span class="icon">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="25"
        ...
        <div class="col-md-5">
          <div class="card">
            ...
            <div class="card-body">
              <table>
                <tbody>
                  {Object.entries(formData).map(([key, value, label]) => (
                    <TableRow
                      key={key}
                      label={key}
